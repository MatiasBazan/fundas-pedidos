...
<div x-data="pedidoEditForm">
<form method="POST" action="{{ route('pedidos.update', $pedido) }}" class="space-y-5">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="fecha" value="{{ old('fecha', $pedido->fecha?->toDateString()) }}"
    ...
    <div class="flex items-center gap-3">
        <button type="submit"
                class="px-6 py-2.5 bg-[#FF2D6B] hover:bg-[#E0245E] text-white rounded-xl text-sm font-semibold transition-all shadow-sm shadow-[#FF2D6B]/30 flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('pedidoEditForm', () => ({
            items: window._itemsInit.length
                ? window._itemsInit
                : [{ stock_id: '', cantidad: 1, precio_unitario: '' }],
    ...
    function toggleTipoPago() { tipoPagoField.classList.toggle('hidden', estadoPago.value !== 'pagado'); }
