    ...
    @forelse($compras as $compra)
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div class="flex gap-2 pt-3 mt-3 border-t border-gray-100">
                ...
                <form method="POST" action="{{ route('compras.destroy', $compra) }}" class="flex-1" onsubmit="return confirm('¿Eliminar esta compra y todos sus items?')">
                    @csrf @method('DELETE')
                    <button class="w-full py-2 text-xs font-semibold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors">Eliminar</button>
                </form>
            </div>
        </div>
    @empty
